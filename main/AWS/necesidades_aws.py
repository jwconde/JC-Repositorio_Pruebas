    ...
    # Check if event has Records key
        ...
        for record in event['Records']:
            ...
            try:
                ...
                string_to_dt_conv = lambda x: pd.to_datetime(x, utc=True) if x != '00000000' else pd.to_datetime(
                    '1990-01-01 00:00:00', format='%Y-%m-%d %H:%M:%S', utc=True)
                float_to_int_conv = lambda x: int(x[0:x.find('.')]) if x else None
                string_to_time_conv = lambda x: pd.to_datetime(x, format='%H%M%S',
                                                               utc=True) if x != '000000' else pd.to_datetime(
                    '1990-01-01 00:00:00', utc=True)
                df = pd.read_csv(file_content,
                                 sep=csv_separator,
                                 header=None,
            ...
            del df['Pto.exped./depto.entrada mcía.']
