...
supplier = 'SAP'
dest = 'ZEUS'
file = 'STOCKS-COBERTURAS'
cleansed_filename = 'stocks-coberturas'
...
def lambda_handler(event, context):
    ...
    # Check if event has Records key
        ...
        for record in event['Records']:
            ...
            float_to_str_conv = lambda x: str(x)
            ...
            try:
                ...
                df = pd.read_csv(file_content,
                                 ...
                                 dtype=column_types,
                                 converters={'Status de Material específico  de centro': float_to_str_conv,
                                             'Stock valorado de libre utilizacion': float_to_int_conv,
                                             'Stock en control de calidad': float_to_int_conv,
                                             'Stock bloqueado': float_to_int_conv,
                                             'Stock en traslado (de almacén a almacén)': float_to_int_conv,
                                             'Stock total de lotes (todos) no libres': float_to_int_conv,
                                             'Stock bloqueado de devoluciones': float_to_int_conv
            ...
            if response['ResponseMetadata']['HTTPStatusCode'] // 100 == 2:
                ...
                return_message = 'Error putting obejct ' + + ' into manage bucket (' + manage_bucket_name + ')'
                return_code = 500
                logger.error(return_message)
                raise Error(return_message)
            ...
            # Fill file STOCK-COBERTURA
            for i in result.items():
                json_result['STOCK-COBERTURA'].append(i[1])
